 ...
 * of the receiving [StatefulWorkflow], which will invoke the given [lambda][block]
 * when it is [applied][WorkflowAction.apply].
 *
 * The returned object will include the string returned from [name] in its [toString].
 *
 * For larger, more state machine like workflows, prefer creating an `enum` or `sealed class` that
