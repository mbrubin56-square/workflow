...
interface WorkflowAction<StateT, out OutputT : Any> {
  ...
  companion object {
     ...
     *
     * If defining actions within a (small) [StatefulWorkflow], use the [action] extension to
     * do this without being forced to repeat its parameter types. For larger, more
     * state machine like workflows, prefer creating an `enum` or `sealed class` that
     * implements the [WorkflowAction] interface directly.
     *
     ...
     */
    @Deprecated("Implement WorkflowAction directly, or use `WorkflowAction.invoke`")
     ...
     */
    @Deprecated("Use `WorkflowAction { output }`")
