...
In the workflow framework, state flows _down_ the tree, and actions come _up_ as output events (as in the traditional computer science trees that grow downward).
...
final class TodoEditViewController: SquareScreenViewController<TodoEditScreen> {
...
First, modify the state to represent if the list is being viewed, or an item is being edited:
...
Then, add actions for selecting a todo item as well as saving or discarding the changes:
...
Finally, update the `render` method to defer to the `TodoEditWorkflow` when editing:
...
Now we have an (nearly) fully formed app! Try it out and see how the data flows between the different workflows:
